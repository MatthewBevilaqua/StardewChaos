...
namespace StardewChaos
{
    public class PongMinigame : IMinigame
    {
        ...
        private const int PaddleH = 110;
        private const int BallSize = 18;
        private const int PaddleMargin = 40;

        private const float PaddleSpeed = 9f;
        private const float CpuSpeed = 6.2f;
        private const float BallSpeed = 8.5f;
        private const float MaxBallSpeed = 16f;
        ...
        public void draw(SpriteBatch b)
        {
            ...
            try
            {
                ...
                DrawRect(b, ballRect, Color.White);

                int halfX = ScreenW / 2;
                for (int y = 20; y < ScreenH - 20; y += 30)
                {
                    DrawRect(b, new Rectangle(halfX - 2, y, 4, 18), Color.DarkGray);
                }

                DrawCenteredText(b, $"PLAYER  {_scoreLeft}   -   CPU  {_scoreRight}", ScreenW / 2, 30, Color.White);
        ...
        public string minigameId() { return "Pong"; }
    }
}